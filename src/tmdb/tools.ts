...
const searchTmdbParams = Type.Object({
	query: Type.String({
		description:
			"The search query to look for across all media types",
	}),
	page: Type.Optional(
		Type.Number({
			description: "Optional: Page number for pagination (default: 1)",
...
export const getTmdbMovieDetailsTool: AgentTool<
	typeof getTmdbMovieDetailsParams
> = {
	...
	async execute(_toolCallId, params) {
		...
		return {
			content: [
				{ type: "text", text: JSON.stringify(toPartialMovieDetails(movie)) },
			],
			details: undefined,
		};
	},
...
export const getTmdbSeriesDetailsTool: AgentTool<
	typeof getTmdbSeriesDetailsParams
> = {
