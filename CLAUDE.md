...
- **Adding new tools**: Create `api.ts` and `tools.ts` under `server/<service>/`. Register tools in `server/agent/index.ts` (add to imports + `allTools` array). If the tool changes something (rather than looking it up), add it to `ACTION_TOOLS` in `server/agent/action-tools.ts` so it shows as a Slack task card. Also add the service to the media architecture list in `server/agent/system-prompt.ts` — the agent won't use tools it doesn't know about. Add a human-readable summary case in the `toolSummary` switch in both `server/routes/api/threads.ts` and `web/src/utils/toolSummary.ts` — without this, the web UI falls back to the raw tool name.
